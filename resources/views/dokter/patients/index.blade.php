            ...
            <div class="card-header py-3">
                <div class="row justify-content-between">
                    ...
                    @if (auth()->user()->role == 'dokter')
                        <form action="{{ route('dokter.search_resep') }}" method="get">
                            <div class="search">
                                <div class="input-group">
                                    <input type="text" class="form-control" name="q"
                                        value="{{ request()->get('q') }}" placeholder="Masukkan kata">
                        ...
                        <tbody>
                            ...
                            @foreach ($patients as $index => $pasien)
                                <tr class="">
                                    ...
                                    @if (auth()->user()->role !== 'admin')
                                        <td class="text-center" style="font-size: 18px; color:blue">{{ $pasien->no_queue }}
                                        </td>
                                    @endif
...
@section('custom_html')
    @foreach ($patients as $pasien)
        <form action="{{ route('dokter.patients.destroy', $pasien->id) }}" id="delete-form-{{ $pasien->id }}" method="post">
            @csrf
            @method('DELETE')
        </form>
    @endforeach
