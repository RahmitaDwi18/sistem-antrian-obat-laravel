...
class PatientController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('dokter.patients.index', compact('patients', 'polys', 'script', 'hideKelola'));
    }
    ...
    public function show2(Patient $patient)
    {
        ...
        // dd($patient);
        return view('dokter.patients.show2', compact('patient'), compact('style', 'script'));
    }
    ...
    {
        $otherPatiens = Patient::whereDate('created_at', Carbon::today())->whereNotNull('recipe')->get();
        $countOtherPatients = count($otherPatiens);
        $countOtherPatients += 1;

        $patient->no_antrian_obat = $countOtherPatients;
        $patient->recipe = $request->recipe;
        $patient->save();

        return redirect()
            ->to(route('dokter.patients.data', $patient->id))
    ...
    public function data(Request $request)
    {
        ...
        $patients = Patient::where('id_poly', $poly->id_poly)->whereDate('created_at', Carbon::today())
            ->when($request->masuk, function ($query) use ($request) {
                return $query->whereDate('created_at', $request->masuk);
            })
            ->whereNotNull('recipe')
        ...
        return view('dokter.patients.data', compact('patients', 'polys', 'script', 'hideKelola'));
    }
}
